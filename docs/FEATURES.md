# Routiform — Dashboard Features Gallery
...
Visual guide to every section of the Routiform dashboard.

---

## 🔌 Providers

Manage AI provider connections: OAuth providers (Claude Code, Codex, Antigravity), API key providers (Groq, DeepSeek, OpenRouter, DeepInfra, SambaNova, Venice.ai), and free providers (Qoder, Qwen, Kiro). Kiro accounts include credit balance tracking — remaining credits, total allowance, and renewal date visible in Dashboard → Usage.
...
Dashboard for discovering and managing CLI agents. Shows a grid of 13 built-in agents (Codex, Claude, Goose, OpenClaw, Aider, OpenCode, Cline, Qwen Code, ForgeCode, Amazon Q, Open Interpreter, Cursor CLI, Warp) with:

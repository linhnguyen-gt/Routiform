...
Routiform is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Routiform Runtime]
        Next[Next.js Server\nPORT=20128]
...
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DefaultExecutor`     | OpenAI, Claude, Gemini, Qwen, Qoder, OpenRouter, GLM, Kimi, MiniMax, DeepSeek, DeepInfra, SambaNova, Venice.ai, Groq, xAI, Mistral, Perplexity, Together, Fireworks, Cerebras, Cohere, NVIDIA | Dynamic URL/header config per provider                                                                                                                                                                                                                                                                                                                                                  |
| `AntigravityExecutor` | Google Antigravity                                                                                                                                                                            | Custom project/session IDs, Retry-After parsing                                                                                                                                                                                                                                                                                                                                         |
| `CodexExecutor`       | OpenAI Codex                                                                                                                                                                                  | Uses shared token-refresh mutex path, converts native passthrough `input` role `system` → `developer` on both passthrough and non-passthrough paths, injects fallback instructions when missing, strips unsupported token-cap aliases, strips stored-item references (`stripStoredItemReferences`), defaults `store=true`, injects `originator`/`session_id`/`prompt_cache_key` headers |
...
| Gemini           | gemini           | API Key               | ✅               | ✅         | ❌            | ⚠️ Cloud Console   |
...
- final client response returned by Routiform; streamed responses are stored in the same compact summary form
...
1. `usageDb` and `localDb` share the same base directory policy (`DATA_DIR` -> `XDG_CONFIG_HOME/routiform` -> `~/.routiform`) with legacy file migration.
...
5. The `open-sse/` directory is published as the `@routiform/open-sse` **npm workspace package**. Source code imports it via `@routiform/open-sse/...` (resolved by Next.js `transpilePackages`). File paths in this document still use the directory name `open-sse/` for consistency.
...
- Build Docker image: `docker build -t routiform .`

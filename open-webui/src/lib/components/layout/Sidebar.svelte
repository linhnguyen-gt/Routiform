...
{#if !$mobile && !$showSidebar}
	...
	>
		...
		>
			...
			<!-- ROUTIFORM PATCH: the way back out.
			     /owui is a full-screen SPA mounted inside Routiform, and upstream has no concept of
			     a host app to return to — so without this, clicking "Chat" in Routiform's sidebar is
			     a one-way trip and the browser's Back button is the only exit.
			     A plain <a> on purpose: /dashboard is a Next route, not a SvelteKit one, so this must
			     be a real document navigation. 'dashboard' is deliberately NOT in the codemod's ROOTS
			     list (vite-plugin-routiform-base.js), so this href is left alone rather than being
			     rewritten to /owui/dashboard. -->
			<div class="">
				<Tooltip content="Back to Routiform" placement="right">
					<a
						class=" cursor-pointer flex rounded-xl hover:bg-gray-100 dark:hover:bg-gray-850 transition group"
						href="/dashboard"
						draggable="false"
						aria-label="Back to Routiform"
						on:click={(e) => {
							// This whole rail is wrapped in a <button> that toggles the sidebar, so a
							// click here bubbles into it: the sidebar would swing open and only THEN
							// would the page navigate away. Stop the bubble but let the default
							// navigation run — upstream's New Chat link does the same for the same
							// reason.
							e.stopPropagation();
						}}
					>
	...
	>
		...
		>
			...
			>
				...
				<!-- ROUTIFORM PATCH: the way back out, for the EXPANDED sidebar.
				     The icon rail above ({#if !$mobile && !$showSidebar}) only renders while the
				     sidebar is COLLAPSED, so the back link there disappears the moment you open the
				     sidebar — which is the default state. It must exist in both. -->
				<Tooltip content="Back to Routiform" placement="bottom">
					<a

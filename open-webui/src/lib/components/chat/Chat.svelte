<script lang="ts">
	...
	// ROUTIFORM PATCH: see the pageSubscribe below. SvelteKit's `base` is '/owui' for us.
	...
	onMount(() => {
		...
		const pageSubscribe = page.subscribe(async (p) => {
			// ROUTIFORM PATCH. Upstream compares against '/' because the app owns the site root.
			// Ours is mounted at /owui, and SvelteKit puts `base` INSIDE $page.url.pathname — so
			// this was never equal to '/', initNewChat() never ran, and the chat opened with no
			// model selected and a composer that refused to send until you picked one by hand,
			// on every single load. The codemod rewrites goto()/href navigation but cannot see a
			// pathname COMPARISON, which is why this needed a patch of its own.

<script>
	...
	const setupSocket = async (enableWebsocket) => {
		// ROUTIFORM PATCH (see plans/260714-0952-embed-open-webui-spa/plan.md).
		// Upstream WEBUI_BASE_URL is '' so io('') hits the default namespace at the site root.
		// Ours is '/owui', and a leading-slash URL is a socket.io NAMESPACE, not a base path:
		// io('/owui') would still transport over '/ws/socket.io' at the ROOT of the site —
		// outside the /owui prefix that next.config.mjs rewrites and src/proxy.ts guards, so
		// the handshake would 404. Pass no URL (default namespace, this origin) and fold the
		// base into `path` instead, which keeps every Open WebUI request under /owui.
		// Only correct for a built SPA; in `vite dev` WEBUI_BASE_URL is an absolute URL and
		// this would produce a nonsense path — Routiform never runs that dev server.

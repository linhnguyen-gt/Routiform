...
IMAGE_NAME="${1:-routiform-local-hardened}"
CONTAINER_NAME="${2:-routiform-hardening-test}"
HOST_PORT="${3:-20129}"
...
docker build -t "${IMAGE_NAME}" "${WORKDIR}" >/tmp/routiform_hardening_build.log
echo "      Build done."
...
docker run -d \
  ...
  -e DATA_DIR=/app/data \
  "${IMAGE_NAME}" >/tmp/routiform_hardening_container_id.txt

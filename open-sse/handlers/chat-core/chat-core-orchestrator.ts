...
import { applyPiiToResponse } from "./chat-core-pii-response.ts";
...
export async function runChatCoreOrchestrator(
  ...
  const emit = async <T extends { response?: unknown }>(result: T): Promise<T> =>
    applyPiiToResponse(attachCompressionHeader(result, p.compressionHeader));

  const s1 = await chatCorePhaseSetupAndLogs(p);
  if (s1.done) return await emit(s1.result as ChatCoreHandlerResult);

  const s2 = await chatCorePhaseTranslateAndBundle(p);
  if (s2.done) return await emit(s2.result as ChatCoreHandlerResult);

  const s3 = await chatCorePhaseFirstUpstream(p);
  if (s3.done) return await emit(s3.result as ChatCoreHandlerResult);

  await chatCorePhaseUpstreamOauthRetry(p);

  const s4 = await chatCorePhaseUpstreamErrors(p);
  if (s4.done) return await emit(s4.result as ChatCoreHandlerResult);

  if (!p.stream) {
    const s5 = await chatCorePhaseNonStreamParse(p);
    if (s5.done) return await emit(s5.result as ChatCoreHandlerResult);
    const s6 = await chatCorePhaseNonStreamComplete(p);
    return await emit(s6.result as ChatCoreHandlerResult);
  }

  return await emit((await chatCorePhaseStreamingResponse(p)) as ChatCoreHandlerResult);
}
